...
(function (factory) {
  ...
  var Editor = function () {
    ...
    this.insertLatexEquation = function($editable, value){
      
      var toInsert = '&nbsp;<div class="render_this">'+getLatexEquation(value)+'</div>&nbsp;'
      pasteHtmlAtCaret(toInsert, true);

      if($('body').find('.summernote-rendered-latex').length == 0 )
        $('body').append('<div class="summernote-rendered-latex"></div>');
      
      $('.render_this').bind("DOMSubtreeModified",function(){
        $('body > .summernote-rendered-latex').html(renderLatex($(this).html()));
      });
      $('.render_this').mousemove(function( event ) {
        console.log("ASD")
        $('body > .summernote-rendered-latex').html(renderLatex($(this).html()));
        $('body > .summernote-rendered-latex').css('top',event.pageY+20);
        $('body > .summernote-rendered-latex').css('left',event.pageX+20);
      });
      
      $('.render_this').mouseenter(function(){
        $('body > .summernote-rendered-latex').show();
      });
      $('.render_this').mouseout(function(){
        $('body > .summernote-rendered-latex').hide();
      });
      afterCommand($editable);
    }
    /**
  ...
  var Renderer = function () {
    ...
    var tplButtonInfo = {
      ...
      latex:function(lang){
        var blankEquation = tplIconButton('fa fa-superscript icon-superscript', {
          title: "Create a new equation",
          event: 'insertLatexEquation',
          value:'blank',
        });
        var integral = tplIconButton('integral_icon', {
          event: 'insertLatexEquation',
          title: "Integral Equation",
          value:'integral',
        });
        var quadratic = tplIconButton('fa fa-superscript icon-superscript', {
        ...
        var dropdown = '<div class="dropdown-menu">' +
                         '<div class="note-align btn-group">' +
                           blankEquation + integral + quadratic +
                         '</div>' +
                       '</div>';

        return tplIconButton('latex_icon', {
          title: lang.paragraph.paragraph,
          dropdown: dropdown
        });
      }
...
function renderLatex(value){
  try{
    var latex_html = katex.renderToString(value);
  }catch(e){
    var latex_html = e
  }
  return latex_html;
}
function getLatexEquation(value){
  if(value == 'integral')
    return 'f(x) = \\int_{-\\infty}^\\infty x'
  else if(value == 'quadratic')
    return 'ax^2 + bx + c = 0'
  else
    return 'Please enter your equation here'
}
